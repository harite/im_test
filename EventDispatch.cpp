...
#define MIN_TIMER_DURATION	10	// 10 miliseconds 
...
void CEventDispatch::AddTimer(callback_t callback, void* user_data, uint32_t interval)
{
...
void CEventDispatch::_CheckTimer()
{
	...
	for (it = m_timer_list.begin(); it != m_timer_list.end(); )
	{
		...
		if (curr_tick >= pItem->next_tick)
		{
			...
			pItem->callback(pItem->user_data, NETLIB_MSG_TIMER, 0, curr_tick, NULL);
		}
	}
}
...
void CEventDispatch::StartDispatch()
{
	...
	timeout.tv_sec = 0;
	timeout.tv_nsec = MIN_TIMER_DURATION * 1000;
