...
//CThreadLock	g_map_lock;

void AddBaseSocket(CBaseSocket* pSocket)
{
	//CFuncLock func_lock(&g_map_lock);
...
void CBaseSocket::OnRead()
{
	...
	else
	{
		...
		{
			m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t)m_socket, 0, NULL);
		}
		else
		{
			m_callback(m_callback_data, NETLIB_MSG_READ, (net_handle_t)m_socket, avail, NULL);
		}
	}
}
...
void CBaseSocket::OnWrite()
{
	...
	if (m_state == SOCKET_STATE_CONNECTING)
	{
		...
		m_callback(m_callback_data, NETLIB_MSG_CONFIRM, (net_handle_t)m_socket, 0, NULL);
	}
	else
	{
		m_callback(m_callback_data, NETLIB_MSG_WRITE, (net_handle_t)m_socket, 0, NULL);
	}
}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		log("_AcceptNewSocket(), new socket %d connect in from %s:%d\n", fd, ip_str, port);
		...
		m_callback(m_callback_data, NETLIB_MSG_CONNECT, (net_handle_t)fd, 0, NULL);
	}
}
